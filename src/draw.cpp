...
void drawNormalInterpolation(const Vertex v0, const Vertex v1, const Vertex v2, const glm::vec3 p,
    const glm::vec3 interpolatedNormal, const Ray ray)
{
    if (!enableDebugDraw)
        return;
    ...
    normal0.origin = v0.position;
    normal0.direction = v0.normal;
    normal0.t = 1;

    normal1.origin = v1.position;
    normal1.direction = v1.normal;
    normal1.t = 1;

    normal2.origin = v2.position;
    normal2.direction = v2.normal;
    normal2.t = 1;

    interpolated.origin = p;
    interpolated.direction = interpolatedNormal;
    interpolated.t = 1;

    if (glm::dot(ray.direction, interpolated.direction) > 0) {
        interpolated.direction = -interpolated.direction;
        normal0.direction = -normal0.direction;
        normal1.direction = -normal1.direction;
        normal2.direction = -normal2.direction;
    }

    glm::vec3 colour { 1.0, 1.0, 1.0 };

    drawRay(normal0, colour);
    drawRay(normal1, colour);
    drawRay(normal2, colour);
    drawRay(interpolated, { 0, 0, 1 });
}
