    ...
    if (!config.cliRenderingEnabled) {
        ...
        while (!window.shouldClose()) {
            ...
            if (ImGui::CollapsingHeader("Features", ImGuiTreeNodeFlags_DefaultOpen)) {
                ...
                ImGui::Checkbox("Acceleration Data Traversal", &config.features.accelerationDataTraversal);
            ...
            if (config.features.extra.enableBloomEffect) {
                ...
                ImGui::SliderFloat("Light threshold", &config.features.extra.bloomThreshold, 0.0f, 1.0f);
                ImGui::SliderInt("Box filter size", &config.features.extra.boxSize, 1, 100);
                ImGui::Checkbox("Display bloom only", &config.features.extra.onlyBloomDisplay);
            }
            ...
            if (x)
                ImGui::SliderFloat("x", &config.features.vector.x, -1, 1);
            ImGui::Checkbox("y-axis", &y);
            if (y)
                ImGui::SliderFloat("y", &config.features.vector.y, -1, 1);
            ImGui::Checkbox("z-axis", &z);
            if (z)
                ImGui::SliderFloat("z", &config.features.vector.z, -1, 1);
